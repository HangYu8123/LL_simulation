...
def Training(filename, focus = 0.9, bias = 0, cons = 0, st_f = 0, st_b = 0, st_c = 0):
    ...
    while(flag):
        ...
        while done is False:
            ...
            if steps % 10 == 0:
                feedback = rescalors.rescale_binary(oracle.get_mean(obs[None, :]), agent.get_mean(obs[None, :])) 
                # print("***********************")
                # print(feedback)
                feedback = rescalors.dynamic_feedback_binary(feedback, focus)
                # print(feedback)
                # print("***********************")
...
Training("TAMER_binary", focus = 1)
#Training("TAMER_binary\\B", bias = 1, st_b = -2/200000)
Training("TAMER_binary\\F", focus = 1, st_f = -0.2/200000)
#Training("TAMER_binary\\C", cons = 1, st_c = -1/200000)
